...
def read_orden(orden_id: str, db: Session = Depends(get_db)):
...
def update_orden(orden_id: str, orden: OrdenCompraUpdate, db: Session = Depends(get_db)):
...
def delete_orden(orden_id: str, db: Session = Depends(get_db)):
