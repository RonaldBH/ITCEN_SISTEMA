...
def read_vale(vale_id: int, db: Session = Depends(get_db)):
...
def update_vale(vale_id: int, vale: ValeConsumoUpdate, db: Session = Depends(get_db)):
...
def delete_vale(vale_id: int, db: Session = Depends(get_db)):
